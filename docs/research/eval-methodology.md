...
6. **RAG-specific judge methodology.** ARES fine-tunes lightweight judges on synthetic RAG data and — critically for small golden sets — reports **confidence intervals via prediction-powered inference (PPI)** instead of point estimates, which is the right statistical treatment for a ~36-item set. (Note: "bootstrap" appears nowhere in the ARES paper — corrected 2026-08-12 claim-level pass; bootstrap resampling, if used by this harness, is the harness's own choice.) [Saad-Falcon et al., "ARES", arXiv:2311.09476](https://arxiv.org/abs/2311.09476)
...
9. **Definitions.** The RAGAS **arXiv paper** defines three metrics: **faithfulness** = fraction of answer claims entailed by retrieved context; **answer relevance** = similarity between the original question and questions generated from the answer; **context relevance** = how relevant the retrieved context is to the question. **Context precision and context recall are RAGAS library metrics (docs), NOT in the paper** — verified 2026-08-12 claim-level pass (the paper text has zero mentions). The harness's `context recall` gate thus comes from the RAGAS library lineage. [Es et al., "RAGAS", arXiv:2309.15217](https://arxiv.org/abs/2309.15217); [RAGAS docs](https://docs.ragas.io)
...
**Aggregation:** item pass = all deterministic gates pass + judge thresholds met. Report means per intent×locale cell plus overall, with 95% CIs at n=36 (ARES derives intervals via prediction-powered inference, not bootstrap — corrected 2026-08-12; bootstrap resampling is an acceptable harness-side choice but must not be attributed to ARES). [Saad-Falcon et al., arXiv:2311.09476](https://arxiv.org/abs/2311.09476)
...
- `scripts/eval/` in repo: `golden.jsonl`, `run_eval.py` (calls `AskAIUseCase.execute` non-streaming with `debug_mode=true`, fresh conversation IDs, fixture account tokens), `score.py` (deterministic gates + judge calls through existing `LLMPort`), `report.py` (per-cell means + 95% CIs).
...
- Saad-Falcon et al., *ARES* (NAACL 2024) — https://arxiv.org/abs/2311.09476 — judge methodology + confidence intervals via prediction-powered inference (PPI; not bootstrap — corrected 2026-08-12).
