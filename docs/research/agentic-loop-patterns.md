...
2. **Primary-source evidence**: the ReAct paper (ICLR 2023) shows interleaved reasoning+acting **reduces hallucination** vs CoT on HotpotQA and FEVER (ReAct hallucination 0% vs CoT 56% on sampled HotpotQA trajectories; abstract: ReAct "overcomes issues of hallucination and error propagation prevalent in chain-of-thought reasoning"). On accuracy: ReAct beats acting-only on both tasks (HotpotQA 27.4 vs 25.7 EM; FEVER 60.9 vs 58.9 acc) and beats CoT on FEVER (60.9 vs 56.3), but **slightly lags plain CoT on HotpotQA (27.4 vs 29.4 EM)**; the combined ReAct→CoT-SC reaches 35.1 vs CoT-SC 33.4 (Yao et al., Table 1). [Yao et al., "ReAct", arXiv:2210.03629](https://arxiv.org/abs/2210.03629) `[VERIFIED via project pass + claim-level pass]`. Provider-side support is documented by both vendors: Cohere's tool-use docs (https://docs.cohere.com/docs/tool-use — "built-in tool use capabilities such as function calling, multi-step reasoning") and Google's Gemini function-calling docs (https://ai.google.dev/gemini-api/docs/function-calling); Google's Agents whitepaper also frames ReAct as a core agent pattern (https://www.kaggle.com/whitepaper-agents).
...
2. **Primary-source evidence**: the paper reports gains on fact verification (PubHealth) and open-domain QA vs **ChatGPT and retrieval-augmented Llama2-chat** (the paper's named baselines — standard-RAG-style, not "CoT" per se), with fewer hallucinated statements; critics are the core mechanism. Critically for this stack: the trained control-token/critic models are English-trained.
3. **Latency/token cost**: reflection adds critique tokens to every generation, and the mechanism adds compute (K passages processed per segment, beam width 2). Note: the paper itself **does not quantify inference cost** vs plain RAG (verified: no "inference cost"/"latency" claims in the arXiv text; its only efficiency discussion is the retrieval-frequency threshold trade-off) — the cost increase is an engineering inference, not a paper claim. In a prompt-based (non-fine-tuned) implementation here, it becomes an extra verification LLM call per turn — a real cost on both providers.
...
1. **How it works**: after retrieval, a retriever-evaluator grades retrieval confidence (the paper's lightweight retrieval evaluator — a fine-tuned T5-large, 0.77B; the name "TIGER" does not appear in the paper); on low confidence the system takes corrective actions — query rewriting + **web search fallback**, knowledge refinement (decompose-then-recompose), or regeneration. [Yan et al., "Corrective Retrieval Augmented Generation", arXiv:2401.15884](https://arxiv.org/abs/2401.15884) `[VERIFIED via project pass + claim-level pass]`
2. **Primary-source evidence**: CRAG reports gains over standard RAG on four datasets — **PopQA, Biography, PubHealth, Arc-Challenge** (e.g., +7.0% acc PopQA, +14.9 FactScore Biography, +36.6% acc PubHealth, +15.4% acc Arc-Challenge on SelfRAG-LLaMA2-7b) — and specifically analyzes the imperfect-retrieval regime, the exact "KB incomplete" failure this stack lives with. (Note: the datasets are NOT KILT-family; KILT covers NQ/HotpotQA/FEVER/Feverous/T-REx/zsRE/WoW/ELI5/AIDA/WNED-WIKI — earlier drafts of this brief had the wrong list.) The web-fallback path maps 1:1 onto the existing `search_trusted_web` tool.
...
5. **Low-resource fit**: the T5-large evaluator is English-trained (fine-tuned on PopQA relevance signals); the **deterministic threshold variant is language-agnostic** (it grades scores, not text) — the recommended route for Amharic. LLM-judge variant needs an Amharic-capable judge (Gemini flash fits).
...
2. **Primary-source evidence**: RARR reports significant attribution improvements — headline figure: "After editing, RARR can increase attribution by **up to 13% absolute**, while changing only 10–20% of the text" (§5.3); the abstract contains no numbers, and there is no "+20 points" figure (earlier draft error). This is the only pattern in the list whose *entire point* is the citation-groundedness metric the harness gates on (citation coverage + grounding precision).
...
- Saad-Falcon et al., *ARES* — https://arxiv.org/abs/2311.09476 — judge methodology + confidence intervals via prediction-powered inference (PPI; NOT bootstrap — corrected in claim-level pass). `[VERIFIED via project pass + claim-level pass]`
...
3. **Amharic tool-calling quality** on Cohere `command-a-03-2025` vs the current Gemini 3.x lineup is unverified — the A/B's AM cells plus a 5-item human spot-check of AM judge outputs (eval-methodology.md Gap 3) are the only evidence; plan for it.
...
## Claim-level verification pass (live web, 2026-08-12)

After the ID-level pass (above), a second live pass checked the **content of every load-bearing claim** against the primary sources (arXiv abstract + paper HTML bodies + official docs) — not just the IDs. Full report: `.pi-subagents/artifacts/outputs/1253c20a/research.md`. Verdict distribution: **VERIFIED × 12, CORRECTED × 6** (all corrections already applied inline above; record kept here):

| Claim | Verdict | Evidence (quoted) |
|---|---|---|
| ReAct (2210.03629): beats CoT on HotpotQA+FEVER | **CORRECTED** | ReAct "overcomes issues of hallucination…" but "slightly lags behind CoT on HotpotQA (27.4 vs. 29.4 EM)"; beats CoT on FEVER (60.9 vs 56.3); hallucination 0% vs CoT 56% (Table 2) |
| Plan-and-Solve (2305.04091): prompting paper | VERIFIED | "zero-shot prompting consistently outperforms Zero-shot-CoT across all datasets" — pure prompting, no tools |
| Adaptive-RAG (2403.14403): T5 complexity router | VERIFIED | "we use and train the T5-Large model"; routes no-retrieval / single-step / multi-step; exceeds single-step RAG on avg (EM 37.17 vs 34.83) |
| Self-RAG (2310.11511): reflection tokens + gains | VERIFIED (main) / CORRECTED (cost) | Retrieve/IsRel/IsSup/IsUse verified; beats ChatGPT + retrieval-augmented Llama2-chat; **"paper reports higher inference cost" NOT in paper** — engineering inference only |
| CRAG (2401.15884): "TIGER" evaluator | **CORRECTED** | Evaluator is a fine-tuned **T5-large (0.77B)**; "TIGER" appears nowhere in v1/v3 |
| CRAG: KILT-family datasets | **CORRECTED** | Evaluated on **PopQA, Biography, PubHealth, Arc-Challenge** (+7.0%/+14.9 FactScore/+36.6%/+15.4% vs RAG) — not SciFact/Feverous/NQ/HotpotQA, none KILT |
| RARR (2210.08726): +20 points attribution | **CORRECTED** | Headline figure: "up to **13% absolute**… changing only 10–20% of the text" (§5.3); abstract has no numbers |
| τ-bench (2406.12045): agents <50% | VERIFIED | "even state-of-the-art function calling agents (like gpt-4o) succeed on <50% of the tasks" |
| LLM+P (2304.11477): PDDL planner | VERIFIED | "converting the language description into… PDDL, then leveraging classical planners" (executor is a classical planner, not an LLM — looser framing noted) |
| RAG (2005.11401): one retrieval + one generation | VERIFIED | Accurate for RAG-Sequence; RAG-Token re-retrieves per token (operationalization noted) |
| ALCE (2304.09848): claim-level citation recall/precision | VERIFIED | "high citation recall… high citation precision" — sentence/statement level |
| RAGAS (2309.15217): 4 metrics | **CORRECTED** | Paper defines **three**: Faithfulness, Answer Relevance, Context Relevance; context precision/recall are **library** metrics (0 mentions in paper) |
| ARES (2311.09476): bootstrap CIs | **CORRECTED** | Intervals via **prediction-powered inference (PPI)**; zero "bootstrap" matches in the paper |
| XNLI (1809.05053): 15 langs, no Amharic | VERIFIED | Full list quoted; 0 "Amharic" matches |
| Cohere tool-use docs | VERIFIED | "Cohere's Command family… built-in tool use capabilities such as function calling, multi-step reasoning, and citation generation" |
| Gemini function-calling docs | VERIFIED | "connect models to external tools and APIs… model determines when to call specific functions" (via search-indexed copy; direct fetch blocked) |
| Google Agents whitepaper: ReAct core pattern | VERIFIED | "reasoning and logic frameworks, like ReAct, Chain-of-Thought, or Tree-of-Thoughts" (via PDF mirror; JS-rendered) |
| LangGraph plan-and-execute tutorial | VERIFIED | "plan-and-execute approach is useful for tackling complex tasks by first establishing a long-term plan" |

**Impact on the recommendation**: none of the corrections change the architecture verdict (keep ReAct + route + gate + pre-stream verify) — but three matter for how the A/B is *argued and cited*: (a) the CRAG gate (A2) should cite the evaluator as T5-large-based with PopQA/Biography/PubHealth/Arc-Challenge results, not "TIGER/KILT"; (b) the pre-stream verify (A3) should cite RARR's "up to 13% absolute" figure; (c) the harness's statistical method must not attribute bootstrap to ARES (PPI is the paper's method; bootstrap is a harness-side choice). The ReAct correction (slightly lags CoT on HotpotQA) is a nuance for the map's "planning" discussion, not for the loop decision.
